    ...
    for (original, include_path, block_text, snippet_text, ppinclude_path) in py_elements:
        ...
        if len(include_path) > 0:
            ...
            if ppinclude_path.startswith("/"):
                ppinclude_path = join(gen_dir, ppinclude_path)
            else:
...
output_template = "$doc"
try:
    output_template = open(join(gen_dir, ".doctemplate.html")).read()
...
def generate(folder):
    ...
    for path in listdir(folder):
        ...
        # Recurse into subdirectories later
        if isdir(full_path):
    ...
    # Do subdirectories after files in the current directory
