...
def generate_file_from_template(source_path, output_path, vars):
    source_path = f"{root_dir}/{source_path}"
    output_path = f"{root_dir}/{output_path}"
...
# note: automatically handles file extension
def get_vars_listing(path, base_folder=info_folder, list_file=listing_file_name):
    listing_path = f"{base_folder}{path}/{list_file}{info_extension}"
...
mdformat_rules = {
    'list': [r"regex <<([\s\S]+?)>>", '<ul>$1</ul>'],
    'list-item': [r"regex \s*--\s*(.*)", '<li>$1</li>'],

    'paragraph': [r'regex (?:\n\s*|^\s*)([^!#\s][\s\S]*?)(?=\n\n|\n*$)', '<p>$1</p>\n'],

    'subsubheading': ['### ? ###', '<h3>$1</h3>'],
    'subheading': ['## ? ##', '<h2>$1</h2>'],
    'heading': ['# ? #', '<h1>$1</h1>'],
